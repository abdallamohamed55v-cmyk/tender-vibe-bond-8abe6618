// Ported from Open-Generative-AI (https://github.com/Anil-matcha/Open-Generative-AI)
// Used to enrich prompts in Image / Video / Cinema studios and the Marketing Studio.
...
/**
 * Compile a cinematic prompt for Nano Banana / Flux-style image models.
 */
...
/**
 * Lightweight prompt enhancer — appends one tag from each category.
 * For deeper rewriting, use the AI enhancer in `marketing-generate` edge function.
 */
export function quickEnhancePrompt(basePrompt: string): string {
  const pick = <T,>(arr: readonly T[]) => arr[Math.floor(Math.random() * arr.length)];
  const tags = [pick(ENHANCE_TAGS.quality), pick(ENHANCE_TAGS.lighting), pick(ENHANCE_TAGS.mood), pick(ENHANCE_TAGS.style)];
  return `${basePrompt.trim()}, ${tags.join(", ")}`;
}
